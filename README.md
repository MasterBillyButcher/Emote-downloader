...
risk matters to you, keep `ACCESS_CODE` set so the Twitch Subscriber Emotes feature (the one that
uses your Developer credentials) isn't wide open to the public internet.

## Security headers

`next.config.mjs` sets `X-Frame-Options`, `X-Content-Type-Options`, `Cross-Origin-Opener-Policy`,
`Strict-Transport-Security`, and a `Content-Security-Policy` on every route. These were added
after a Lighthouse audit flagged their absence as high-severity findings.

One honest tradeoff: the CSP's `script-src` includes `unsafe-inline`. Next.js injects small inline
hydration scripts into every page, and without `unsafe-inline` (or per-request nonces, which need
custom middleware) those get blocked and the app breaks. This is a real gap, not a mistake: a
nonce-based CSP is the stricter option if you want to close it, at the cost of adding middleware.
