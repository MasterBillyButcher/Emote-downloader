...
// ---------- Twitch chat badges (subscriber loyalty + Bits/cheer) ----------
...
// confirming Twitch's current policy. Both subscriber and Bits badges come
// from this same one call, so it's fetched once and shared.
async function fetchRawTwitchBadgeSets(broadcasterId, clientId, clientSecret) {
  const token = await getTwitchAppToken(clientId, clientSecret);
...
function badgeSetToEmotes(data, setId, namePrefix, source) {
  const set = data.find((s) => s.set_id === setId);
  const versions = set ? set.versions : [];
  return versions.map((v) => ({
    source,
    name: `${namePrefix}_${v.id}`,
    animated: false,
    ext: "png",
...
export async function listTwitchBadges(broadcasterId, clientId, clientSecret) {
  const data = await fetchRawTwitchBadgeSets(broadcasterId, clientId, clientSecret);
  const emotes = badgeSetToEmotes(data, "subscriber", "subscriber", "twitch-badges");
...
export async function listTwitchCheerBadges(broadcasterId, clientId, clientSecret) {
  const data = await fetchRawTwitchBadgeSets(broadcasterId, clientId, clientSecret);
  const emotes = badgeSetToEmotes(data, "bits", "cheer", "twitch-cheer-badges");
  return { emotes, rawTotal: emotes.length };
}

export async function fetchTwitchCheerBadgesToArchive(archive, broadcasterId, clientId, clientSecret, nameFilter) {
  const { emotes: rawEmotes } = await listTwitchCheerBadges(broadcasterId, clientId, clientSecret);
  const emotes = nameFilter ? rawEmotes.filter((e) => nameFilter.has(e.name)) : rawEmotes;
  return mapWithConcurrency(emotes, 8, (e) =>
    appendFetchedFile(archive, e.downloadUrl, "twitch-cheer-badges", e.name, e.ext, undefined, e.source)
  );
}

// ---------- Twitch Cheermotes ----------
// A different endpoint again (bits/cheermotes) - Twitch's own docs describe
// it as usable "in any Bits-enabled channel's chat room," which reads as a
// broadly public endpoint like the emote ones, not restricted the way
// badges is. includeGlobal reuses the same meaning it has for 7TV/BTTV/FFZ:
// off means only this channel's own custom Cheermote tier set (if it has
// one), on also includes Twitch's global default Cheer/tier icons that are
// identical on every channel.
export async function listTwitchCheermotes(broadcasterId, clientId, clientSecret, includeGlobal) {
  const token = await getTwitchAppToken(clientId, clientSecret);
  const res = await fetch(`https://api.twitch.tv/helix/bits/cheermotes?broadcaster_id=${broadcasterId}`, {
    headers: { "Client-Id": clientId, Authorization: `Bearer ${token}` },
  });
  if (!res.ok) throw new Error(`Twitch Cheermotes lookup failed (HTTP ${res.status})`);
  const { data } = await res.json();

  const raw = includeGlobal ? data : data.filter((c) => c.type === "channel_custom");

  const emotes = [];
  for (const cheermote of raw) {
    for (const tier of cheermote.tiers) {
      const animatedUrl = tier.images?.dark?.animated?.["4"] || tier.images?.dark?.animated?.["2"];
      const staticUrl = tier.images?.dark?.static?.["4"] || tier.images?.dark?.static?.["2"];
      const animated = Boolean(animatedUrl);
      const downloadUrl = animatedUrl || staticUrl;
      if (!downloadUrl) continue;
      emotes.push({
        source: "twitch-cheermotes",
        name: `${cheermote.prefix}${tier.min_bits}`,
        animated,
        ext: animated ? "gif" : "png",
        previewUrl: (animated ? tier.images?.dark?.animated?.["2"] : tier.images?.dark?.static?.["2"]) || downloadUrl,
        downloadUrl,
      });
    }
  }
...
export async function fetchTwitchCheermotesToArchive(
  archive,
  broadcasterId,
  clientId,
  clientSecret,
  ...
  const { emotes: rawEmotes } = await listTwitchCheermotes(broadcasterId, clientId, clientSecret, includeGlobal);
  const emotes = nameFilter ? rawEmotes.filter((e) => nameFilter.has(e.name)) : rawEmotes;
  return mapWithConcurrency(emotes, 8, (e) =>
    appendFetchedFile(archive, e.downloadUrl, "twitch-cheermotes", e.name, e.ext, undefined, e.source)
  );
}
